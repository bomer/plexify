...
**Last updated:** 10 August 2026 · **62 complete**
...
| 64 | Home renders Plex's own hubs | Net 533 lines deleted. `/hubs/sections` publishes eleven rows for James's library and three of them were exactly what this app had reimplemented: more by an artist, more in a genre, most played in a month. Both bugs found on 10 August lived in those reimplementations and neither could exist here, because the server names, fills and orders the row. Nothing is keyed on an identifier, so a server upgrade can only add rows. Jump back in, Recently added, Favourites and Buried treasure stay local so a cold or offline start still has a Home screen |
